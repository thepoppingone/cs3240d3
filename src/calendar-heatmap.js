...
function calendarHeatmap() {
  ...
  function chart() {
    ...
    // Remove default dateRange
    // var startOfCommit = testChartData[0].date;
    // var endOfCommit = testChartData[testChartData.length-1].date;

    var dateRange = d3.time.days(yearAgo, now); // generates an array of date objects within the specified range

    // var dateRange = d3.time.days(startOfCommit, endOfCommit);
    ...
    // var monthRange = d3.time.months(moment(startOfCommit).startOf('month').toDate(), endOfCommit); // it ignores the first month if the 1st date is after the start of the month
